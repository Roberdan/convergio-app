...
export interface ModelMetric { model: string; latency_ms: number; cost_per_1k_tokens: number; quality_score: number }
export interface RoutingDecision {
  selected_model: string; effective_tier: string; reason: string;
  fallback_chain?: string[]; budget_remaining?: number | null;
}
export interface RoutingResponse { decision: RoutingDecision | string; model_metrics: ModelMetric[] }
...
export interface CostPerHour { avg: number; max: number; min: number }
export interface ObservatoryDashboard {
  cost_per_hour: CostPerHour | number | null;
  tasks_per_day: number | null;
  avg_latency_ms: number | null;
  model_breakdown?: Record<string, number>;
  task_throughput?: Array<{ date: string; org_id: string; tasks_completed: number; tasks_failed: number }>;
  model_latency?: unknown[];
  ok?: boolean;
}
...
/* ── Security CRUD types (used by security tab components) ── */
...
export type { TrustLevel, SecretFilter, KnowledgeEntry, KnowledgeWriteInput, Report, ReportGenerateInput, NodeReadiness, IpcEvent, DeployStatus, DeployHistoryEntry, RetentionRule, DoctorSummary, DoctorIssue, DoctorFullResult, SchedulerPolicy, SchedulerDecision, SchedulerDecideInput, SchedulerUpdateInput, VersionInfo, Capability, Notification, NotifySendInput, BillingAlert, BillingAlertInput } from './types-ext';
