...
import type {
  HealthResponse, DeepHealthResponse, MetricsResponse, RuntimeView,
  Agent, AgentInput, UsageResponse, Invoice, RateCard, AlertRequest,
  CostSummary, RoutingResponse, TimelineEvent, SearchResult,
  ObservatoryDashboard, Anomaly, Prompt, PromptInput, Skill, SkillInput,
  Org, OrgInput, OrgDelegation, Plan, PlanInput, Wave, MeshNode,
  ExtensionInfo, DepGraph, DepGraphValidation, PeerEntry, AuditEntry,
  ResourceStatus, Snapshot,
} from './types';

// Client-side: proxy via Next.js rewrite (same-origin, no CORS)
...
type CostResp = { summaries: CostSummary[] } | CostSummary[];
const unwrapCosts = (r: CostResp) => Array.isArray(r) ? r : (r as { summaries: CostSummary[] }).summaries ?? [];
export const inferenceCosts = (p?: { agent_id?: string; org_id?: string; plan_id?: string }) =>
  get<CostResp>(`/api/inference/costs${qs(p ?? {})}`).then(unwrapCosts);
...
export const observatoryTimeline = (p?: {
  org_id?: string; source?: string; event_type?: string;
  node_id?: string; since?: string; until?: string; limit?: number;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
}) => get<any>(`/api/observatory/timeline${qs(p ?? {})}`).then((r) => {
  const raw: Record<string, unknown>[] = Array.isArray(r) ? r : r?.events ?? [];
  return raw.map((e) => ({
    id: String(e.id ?? ''), timestamp: (e.timestamp ?? e.created_at ?? '') as string,
    org_id: (e.org_id ?? undefined) as string | undefined,
    source: (e.source ?? e.actor ?? '') as string,
    event_type: (e.event_type ?? '') as string,
    node_id: (e.node_id ?? undefined) as string | undefined,
    payload: (e.details_json ?? e.payload ?? {}) as Record<string, unknown>,
  })) as TimelineEvent[];
});

export const observatorySearch = (q: string, limit?: number) =>
  get<{ results: SearchResult[] } | SearchResult[]>(
    `/api/observatory/search${qs({ q, limit })}`,
...
export const observatoryAnomalies = (p?: {
  ...
  get<{ anomalies: Anomaly[] } | Anomaly[]>(
    `/api/observatory/anomalies${qs(p ?? {})}`,
  ).then((r) => (Array.isArray(r) ? r : r?.anomalies ?? []));
...
export const orgList = () =>
  get<{ orgs: Org[] }>('/api/orgs').then((r) => r.orgs);
...
export const backupSnapshots = () =>
  get<{ snapshots: Snapshot[] }>('/api/backup/snapshots').then((r) => r.snapshots);

export const backupCreate = (label?: string) =>
  post<Snapshot>('/api/backup/snapshots/create', { label });

export { orgAsk } from './api-ext';
