...
import { usePlansLocale } from './plans-i18n';
...
export default function PlansPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <MnSectionCard title={t.allPlans} badge={(plans ?? []).length} collapsible defaultOpen>
        <MnDataTable
          columns={PLAN_COLS}
      ...
      {selectedPlan && tree && (
        <MnSectionCard title={`${tree.plan.name} — ${t.executionTree}`} collapsible defaultOpen>
          <div className="space-y-1 p-4">
            <div className="mb-3 flex items-center gap-3">
              <MnProgressRing value={tree.plan.tasks_done} max={tree.plan.tasks_total || 1} size="sm" label={t.progress} />
              <span className="text-sm">{tree.plan.tasks_done}/{tree.plan.tasks_total} {t.tasks}</span>
            ...
            {tree.waves.length === 0 && (
              <p className="text-sm text-muted-foreground">{t.noWaves}</p>
            )}
          </div>
        </MnSectionCard>
      )}

      <MnModal open={showCreate} onOpenChange={setShowCreate} title={t.createPlan}>
